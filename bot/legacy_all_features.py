"""Legacy full-feature implementation.

This file contains the cleaned, full 'all features' bot logic moved from the monolith.
Feature modules call into this file for now. You can later migrate code piece-by-piece
into individual feature modules without changing the public command surface.

NOTE: This is intentionally a transitional layer to get you a working modular project
immediately, with clean entrypoint, migrations, logging, and grouping.
"""
...
# Postgres SSL mode:
# - If DATABASE_SSL is "require"/"true"/"1", asyncpg will require SSL.
# - If DATABASE_SSL is "disable"/"false"/"0", SSL is disabled.
DATABASE_SSL = os.getenv("DATABASE_SSL", "").strip().lower()

def asyncpg_ssl_arg():
    if DATABASE_SSL in {"require", "true", "1", "yes", "y"}:
        return "require"
    if DATABASE_SSL in {"disable", "false", "0", "no", "n"}:
        return None
    # If not specified, leave it to the platform/URL; Railway often works without extra flags,
    # but if you see connection errors, set DATABASE_SSL=require in Railway variables.
    return None
...
async def init_db():
    ...
    db_pool = await asyncpg.create_pool(DATABASE_URL, min_size=1, max_size=10, ssl=asyncpg_ssl_arg())
