...
async def config_system_cmd(
    ...
    ok = await _require_one_action(
        interaction,
        used,
        "Examples: `/config system ping:true` • `/config system health_check:true` • `/config system ` • `/config system ` • `/config system timezone_set:America/Los_Angeles` • `/config system timezone_show:true` • `/config system info:true`",
    )
    if not ok:
        return
...
@bot.tree.command(name="pick_admin", description="(Admin) Add a movie to the Movie Night pool for someone else")
@discord.app_commands.checks.has_permissions(manage_guild=True)
@discord.app_commands.describe(member="Member to attribute this pick to", title="Movie title")
async def pick_admin_cmd(interaction: discord.Interaction, member: discord.Member, title: str):
    if interaction.guild is None:
        await interaction.response.send_message("❌ Must be used in a server.", ephemeral=True)
        return
    ...
    async with db_pool.acquire() as conn:
        # Pool-wide duplicate check (case-insensitive)
        existing = await conn.fetchrow(
            "SELECT 1 FROM movie_pool_picks WHERE guild_id=$1 AND lower(title)=lower($2)",
            guild_id, title
        )
        if existing:
            await interaction.response.send_message(
                f"⚠️ That movie is already in the pool: **{title}**",
                ephemeral=True
            )
            return

        # Insert without a specific conflict target to avoid ON CONFLICT constraint mismatches
        await conn.execute(
            "INSERT INTO movie_pool_picks (guild_id, user_id, title) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING",
            guild_id, user_id, title
        )

    await movie_pool_update_display(interaction.guild)
    await interaction.response.send_message(
        f"✅ Added pick for {member.mention}: **{title}**",
        ephemeral=True
    )
